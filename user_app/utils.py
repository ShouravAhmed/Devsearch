...
def search_profile(request):
    ...
    profiles = Profile.objects.distinct().order_by('created_at').filter(is_verified=True).filter(
        ...
        Q(skill__in = Skill.objects.filter(name__icontains=search_text))
    )
...
def paginate_profile(request, profile_list, profile_per_page, pagination_preview):
    ...
    try:
        ...
        page_no = 1
        current_page_profile_list = paginator.page(1)
    except EmptyPage:
...
def devsearch_email(subject, message, recipient_list):
    send_mail(
        subject,
        message,
...
def account_varification_email(profile_id, user_email):
    ...
    html_content = render_to_string('user_app/varify_account_email_template.html', context)
